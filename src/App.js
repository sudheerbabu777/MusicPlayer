...
class App extends React.Component {
  ...
  onClickDelete = id => {
    ...
    const filterDelete = trackList.filter(each => each.id !== id)
    this.setState({trackList: filterDelete})
  }

  onChangeSearch = event => {
    const {trackList} = this.state
    const filterSearch = trackList.filter(each =>
      each.name.toLowerCase().includes(event.target.value.toLowerCase()),
    )
    this.setState({searchInput: event.target.value, trackList: filterSearch})
  }
          ...
          <div className="play-search-container">
            ...
            <div className="input-container">
              <input
                ...
                value={searchInput}
              />
              <BiSearchAlt2 size="28" color="#ffffff" />
            </div>
          </div>
          {trackList.length !== 0 ? (
            <ul>
              {trackList.map(each => (
                <li className="item" key={each.id}>
                  <div className="pop-container">
                    ...
                    <div>
                      <p className="name-style">{each.name}</p>
                  ...
                  <div className="delete-container">
                    ...
                    <button
                      ...
                      data-testid="delete"
                    >
                      <AiOutlineDelete size="26" color="#ffffff" />
                    </button>
                  </div>
                </li>
              ))}
